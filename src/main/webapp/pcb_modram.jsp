<%-- 
    Document   : pcb_modram
    Created on : 29 mar. 2022, 15:47:41
    Author     : Mauricio
--%>
...
<html>
    ...
    <body>
        ...
        <%
            ...
            int idram = Integer.parseInt(request.getParameter("idram"));
            int idmobo = 0;    
            ...
            try{
                ...
                String strQry1 = "select id_build,id_mobo from cliente "
                ...
                idmobo = Integer.parseInt(rs.getString(2));
                
                String strQry2 = "update pc_builder set id_ram = '"+idram+"' "
        ...
        <div class="divseleccion">
            <div class="seleccionarZzz">
                ...
                <div class="instruccion">
                    ...
                    <div class="instruccionletrasZzz">       
                            ¿Deseas añadir más módulos de memoria RAM?             
                    </div>
                </div>
                ...
                <div class="listaprods">                                              
                    ...
                    <div class="modram">
                        <form name="modulosram" method="post" action="pcb_hdd.jsp">                                      
                            <select class="" name="modulos" required>
            ...
            <%
                ...
                try {
                    bd.conectar();
                    String strQry = "select tam_ram from ram where id_ram = '"+idram+"' ;";
                    ResultSet rs = null;
                    rs = bd.consulta(strQry);
                    
                    String strQry2 = "select maxram_mobo,nram_mobo from mobo where id_mobo = '"+idmobo+"';";
                    ResultSet rs2 = null;
                    ...
                    int cantidad = Integer.parseInt(rs2.getString(1)) / Integer.parseInt(rs.getString(1));
                    int numram = Integer.parseInt(rs2.getString(2));
                    //int cantidad = 128 / 64;
                    
                    if (numram == 1){
                        %><option value="1">No se pueden agregar mas módulos</option><%
                    }
                    else if (numram >= 2 && numram <=8){
                        if(cantidad < 2){
                            %><option value="1">No se pueden agregar mas módulos</option><%
                        }
                        if (cantidad >= 2){
                            %><option value="2">1 Modulo extra (Dual-Channel)</option><%
                        }
                        if (numram >= 4 && numram <=8){
                            if (cantidad >= 4){
                                %><option value="4">3 Modulos extra (Quad-Channel)</option><%
                            }
                            if (numram >= 6 && numram <=8){
                                if (cantidad >= 6){
                                    %><option value="6">5 Modulos extra (Cuadrángulo)</option><%
                                }
                                if (numram >= 6 && numram <=8){
                                    if (cantidad >= 8){
                                        %><option value="8">7 Modulos extra (Cuadrángulo)</option><%
                                    }
                                }
                            }
                        }
            ...
            %>  
                    ...
                    <div class="omitir">
                        <a href="pcb_hdd.jsp?modulos=<%=modulos%>">
                            <input type="button" class="btnseleccionar" value="Omitir">
                        </a>
                    </div>
        ...
        <%
            try{
                ...
                String strQry1 = "select ("
                        ...
                        + "where id_build = '"+idbuild+"' and id_cli = '"+id+"') "
                        + "+ "
                        + "(select (producto.prec_prod) from ram "
